...
arrondissements_paris <- setNames(Map(function(lat, long) c(lat = lat, long = long), latitudes, longitudes),
                            paste0(1:20))

#-------

# Coordonnées de latitude et longitude pour chaque arrondissement
latitudes <- c(43.3000, 43.2995, 43.2957, 43.2892, 43.2901, 43.2880, 43.2827, 43.2711,
               43.2449, 43.3006, 43.2735, 43.3020, 43.3444, 43.3370, 43.3150, 43.3635)
...
arrondissements_marseille <- setNames(Map(function(lat, long) c(lat = lat, long = long), latitudes, longitudes),
                                      paste0(1:16))

# --------

# Coordonnées de latitude et longitude pour chaque arrondissement
latitudes <- c(45.7640, 45.7673, 45.7597, 45.7540, 45.7500, 45.7464, 45.7487, 45.7416, 45.7369)
longitudes <- c(4.8357, 4.8317, 4.8112, 4.8053, 4.8400, 4.8360, 4.8525, 4.8760, 4.8925)

# Création du dictionnaire des arrondissements de Lyon
arrondissements_lyon <- setNames(Map(function(lat, long) c(lat = lat, long = long), latitudes, longitudes),
                                paste0(1:9))
